...
const AnimatedLines = () => {
  // Get the viewport dimensions dynamically
 ...
 const generateNewLine = (): LineItem => {
   ...
   if (!baseCurve.current) {
     ...
     // Use a buffer based on the viewport size to ensure lines start off-screen
     const bufferX = viewport.width / 2 + 15;
     const bufferY = viewport.height / 2 + 15;
     
     switch (startSide) {
       case 0: // Top
       startPoint = new Vector3(Math.random() * viewport.width - viewport.width / 2, bufferY, 0);
       endPoint = new Vector3(Math.random() * viewport.width - viewport.width / 2, -bufferY, 0);
       break;
       case 1: // Bottom
       startPoint = new Vector3(Math.random() * viewport.width - viewport.width / 2, -bufferY, 0);
       endPoint = new Vector3(Math.random() * viewport.width - viewport.width / 2, bufferY, 0);
       break;
       case 2: // Left
       startPoint = new Vector3(-bufferX, Math.random() * viewport.height - viewport.height / 2, 0);
       endPoint = new Vector3(bufferX, Math.random() * viewport.height - viewport.height / 2, 0);
       break;
       case 3: // Right
       startPoint = new Vector3(bufferX, Math.random() * viewport.height - viewport.height / 2, 0);
       endPoint = new Vector3(-bufferX, Math.random() * viewport.height - viewport.height / 2, 0);
       break;
       default:
         startPoint = new Vector3(0, bufferY, 0);
         endPoint = new Vector3(0, -bufferY, 0);
         break;
        }
